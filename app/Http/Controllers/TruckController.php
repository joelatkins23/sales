...
class TruckController extends Controller
{
    public function index()
    {
        ...
        return view('truck.index', compact('lims_truck_all'));
    }
    ...
    public function transactionDetails($id)
    {
        ...
        return view('truck.transaction_details', compact('lims_account_list', 'lims_truck_data','lims_gift_card_list'));
    }
    ...
    {     
        ...
        $get_truck_name = Truck::find($truck_id)->name;
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();
        $sql="select deliveries.reference_no, deliveries.delivery_date, sum(product_sales.total)total, sum(product_sales.qty)qty, products.name,products.id, products.weight
                from deliveries 
                ...
                where deliveries.truck_id='".$truck_id."' and deliveries.status<3 group by id order by delivery_date asc";    
        $lims_truck_data=DB::select(DB::raw($sql));
        return view('truck.day_transaction_details', compact('lims_truck_data','get_truck_name', 'lims_warehouse_list',  'truck_data', 'get_truck_data', 'truck_id', 'start_date','end_date'));
    }    
    public function daytransaction(Request $request){
        ...
        $sql="select deliveries.reference_no, deliveries.delivery_date, sum(product_sales.total)total, sum(product_sales.qty)qty, products.name,products.id, products.weight
                    ...
                    left JOIN product_sales on deliveries.sale_id=product_sales.sale_id
                    left join products on product_sales.product_id=products.id
                    where deliveries.truck_id='".$truck_id."' and deliveries.status<3 and DATE_FORMAT(deliveries.delivery_date,'%Y-%m-%d') BETWEEN '".$start_date."' and '".$end_date."' group by id order by delivery_date asc";    
        $lims_truck_data=DB::select(DB::raw($sql));
    ...
    public function adjuststore(Request $request)
    {
        ...
        return  redirect('trucks')->with('message', 'Data inserted successfully');
    }
    ...
    public function change_status(Request $request){
        ...
        return redirect()->back()->with('message', "Sale Status updated successfully");
    }
