...
class RoleController extends Controller
{
    ...
    public function setPermission(Request $request)
    {
        ...
        if($request->has('customers-delete')){
            ...
            $role->revokePermissionTo('event-index');

        if($request->has('event-index')){
            $permission = Permission::firstOrCreate(['name' => 'event-index']);
        ...
        $role->revokePermissionTo('event-add');

        if($request->has('event-add')){
            $permission = Permission::firstOrCreate(['name' => 'event-add']);
            if(!$role->hasPermissionTo('event-add')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('event-edit');

        if($request->has('event-edit')){
            $permission = Permission::firstOrCreate(['name' => 'event-edit']);
            if(!$role->hasPermissionTo('event-edit')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('event-delete');

        if($request->has('event-delete')){
            $permission = Permission::firstOrCreate(['name' => 'event-delete']);
            if(!$role->hasPermissionTo('event-delete')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('calendar-index');

        if($request->has('calendar-index')){
            $permission = Permission::firstOrCreate(['name' => 'calendar-index']);
        ...
        if($request->has('sales-report')){
            $permission = Permission::firstOrCreate(['name' => 'sales-report']);
            if(!$role->hasPermissionTo('sales-report')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('sales-report');
        if($request->has('purchases-report')){
            $permission = Permission::firstOrCreate(['name' => 'purchases-report']);
            if(!$role->hasPermissionTo('purchases-report')){
                $role->givePermissionTo($permission);
            }
        }
            else
                $role->revokePermissionTo('purchases-report');
