...

{
...

	{
        ...
        $end_date=date('Y-m-d');
		return view('delivery.index', compact('lims_delivery_all','start_date','end_date'));
...

	{
		...
		$lims_delivery_all = Delivery::orderBy('id', 'desc')->whereBetween('created_at',[$start_date, $end_date])->get();
...

    {
        ...
        $delivery->truck_id = $data['truck_id'];
        ...
        $delivery->deliver_date = $data['create_date'];

        $delivery->created_at = $data['create_date'];
...

    {
        ...
        $delivery_data[] = $lims_delivery_data->truck_id;
