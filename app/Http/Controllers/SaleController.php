...
class SaleController extends Controller
{
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            return view('sale.create',compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_account_list', 'lims_pos_setting_data', 'lims_tax_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        if($customer_data->credit_limit) {
            $due_amount = $this->getDueAmount($data['customer_id']);
            if(($due_amount + $data['grand_total']) > $customer_data->credit_limit)
                return redirect()->back()->with('not_permitted', 'Total Due has crossed Credit Limit!');
        }
        $data['user_id'] = Auth::id();
        

        if($data['pos']) {
            $sale_data = Sale::select('reference_no')->where('reference_no', 'LIKE', '%Factura%')->latest()->first();
            if($sale_data) {
                $last_reference_no = explode("-", $sale_data->reference_no);
                $reference_no = 'Factura-' . ($last_reference_no[1] + 1);
            }
            else
                $reference_no = 'Factura-' . (config('pos_invoice_initial_number') + 1);
            ...
            $sale_data = Sale::select('reference_no')->where('reference_no', 'NOT LIKE', '%Factura%')->latest()->first();
            if($sale_data) 
                $data['reference_no'] = $sale_data->reference_no + 1;
            else
                $data['reference_no'] = config('sale_invoice_initial_number') + 1;
        }
    ...
    public function getDueAmount($customer_id)
    {
        ...
        $sale_data = Sale::where([
            ['payment_status', '!=', 4],
            ['customer_id', $customer_id]
        ])->selectRaw(implode(',', $query))->get();
        return $sale_data[0]->grand_total - $sale_data[0]->paid_amount;
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            return view('sale.pos', compact('all_permission', 'lims_customer_list', 'lims_customer_group_all', 'lims_warehouse_list', 'lims_product_list', 'product_number', 'lims_tax_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_brand_list', 'lims_category_list', 'recent_sale', 'recent_draft', 'lims_coupon_list', 'lims_account_list', 'flag'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
