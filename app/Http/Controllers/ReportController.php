...
class ReportController extends Controller
{
    ...
    public function dailySale($year, $month)
    {
        ...
        if($role->hasPermissionTo('daily-sale')){
            ...
            while($start <= $number_of_day)
            {
                ...
                $sale_info_data = Sale::select('sales.reference_no', 'sales.sale_note')->whereDate('created_at', $date)->get();
                $info[$start] = '';
                foreach ($sale_info_data as $key => $info_data) {
                    $info[$start] .= '<strong>Reference: </strong>'.$info_data->reference_no.'<br>'. '<strong>Note: </strong>'.$info_data->sale_note.'<br>';
                }
                ...
                $paid_amount[$start] = $sale_data[0]->paid_amount;
                $start++;
            }
            ...
            $lims_warehouse_list = Warehouse::where('is_active', true)->get();
            $warehouse_id = 0;
            return view('report.daily_sale', compact('total_discount','order_discount', 'total_tax', 'order_tax', 'shipping_cost', 'grand_total', 'paid_amount', 'info','start_day', 'year', 'month', 'number_of_day', 'prev_year', 'prev_month', 'next_year', 'next_month', 'lims_warehouse_list', 'warehouse_id'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function dailySaleByWarehouse(Request $request,$year,$month)
    {
        ...
        while($start <= $number_of_day)
        {
            ...
            $sale_info_data = Sale::select('sales.reference_no', 'sales.sale_note')->where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', $date)->get();
            $info[$start] = '';
            foreach ($sale_info_data as $key => $info_data) {
                $info[$start] .= '<strong>Reference: </strong>'.$info_data->reference_no.'<br>'. '<strong>Note: </strong>'.$info_data->sale_note.'<br>';
            }
    ...
    public function customerReport(Request $request)
    {
        ...
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        $lims_sale_data = Sale::with('biller', 'customer', 'warehouse', 'user')->where('customer_id', $customer_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
