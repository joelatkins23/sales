			    	...
			    	while ($i <= $number_of_day) {
			    		...
			    		for($j=1 ; $j<=7 ; $j++){
			    			...
			    			if($flag){
			    				...
			    				if($paid_amount[$i]) {
			    					echo '<strong>'.trans("file.Paid Amount").'</strong><br><span>'.$paid_amount[$i].'</span><br><br>';
			    					echo '<strong>'.trans("file.Due").'</strong><br><span>'.($grand_total[$i] - $paid_amount[$i]).'</span><br><br>';
			    				}
			    				if($grand_total[$i]){
