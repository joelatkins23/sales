...
<html>
  <head>
    ...
    <link rel="stylesheet" href="<?php echo asset('public/vendor/bootstrap/css/bootstrap-datetimepicker.css') ?>" type="text/css">
    ...
    <script type="text/javascript" src="<?php echo asset('public/vendor/bootstrap/js/bootstrap-datetimepicker.js') ?>"></script>

    <script type="text/javascript" src="<?php echo asset('public/vendor/jquery/bootstrap-datetimepicker.js') ?>"></script>
  ...
  <body onload="myFunction()">
        ...
        <div class="side-navbar-wrapper">
          ...
          <div class="main-menu">
            <ul id="side-main-menu" class="side-menu list-unstyled">                  
              ...
              @endif
              <li><a href="#truck" aria-expanded="false" data-toggle="collapse"> <i class="fa fa-truck"></i><span>{{trans('file.Trucks')}}</span></a>
                <ul id="truck" class="collapse list-unstyled ">
                  <li id="truck-list-menu"><a href="{{route('trucks.index')}}">{{trans('file.Truck List')}}</a></li>
                  <li><a id="add-truck" href=""> {{trans('file.Add Truck')}}</a></li>
                </ul>
              </li>
              <?php 
              ...
              <li><a href="#sale" aria-expanded="false" data-toggle="collapse"> <i class="icon ion-ios-cart-outline"></i><span>{{trans('file.Sale')}}</span></a>
                <ul id="sale" class="collapse list-unstyled ">
                  ...
                  ?>
                  @if($add_permission_active)
                  <li><a href="{{route('sale.pos')}}">Factura</a></li>
              ...
              <li class=""><a href="#account" aria-expanded="false" data-toggle="collapse"> <i class="fa fa-university"></i><span>{{trans('file.Accounting')}}</span></a>
                <ul id="account" class="collapse list-unstyled ">
                  ...
                  @endif
                  <li id="transfer-money-menu"><a id="transfer-money" href="">{{trans('file.Transfer Money')}}</a></li>
                </ul>
              </li>
              @endif
              <?php 
              ...
              <?php 
                $index_permission = DB::table('permissions')->where('name', 'event-index')->first();
                  $index_permission_active = DB::table('role_has_permissions')->where([
                        ['permission_id', $index_permission->id],
                        ['role_id', $role->id]
                    ])->first();
              ...
              <li><a href="#event" aria-expanded="false" data-toggle="collapse"> <i class="ion-clock"></i><span>Programar</span></a>
                <ul id="event" class="collapse list-unstyled ">
                  <li id="event-list-menu"><a href="{{route('event.index')}}">Listado</a></li>
                  <?php 
                  ...
                  ?>
                  @if($add_permission_active)
                  <li id="event-create-menu"><a href="{{route('event.create')}}">Agregar</a></li>
                  @endif
                </ul>
              </li>
              @endif
              <?php 
                $index_permission = DB::table('permissions')->where('name', 'calendar-index')->first();
                  $index_permission_active = DB::table('role_has_permissions')->where([
                        ['permission_id', $index_permission->id],
                        ['role_id', $role->id]
                    ])->first();
              ?>
              @if($index_permission_active)
              <li><a href="{{route('calendar.index')}}" > <i class="ion-calendar"></i><span>Calendario</span></a>
              ...
              <li><a href="#report" aria-expanded="false" data-toggle="collapse"> <i class="icon ion-ios-paper-outline"></i><span>{{trans('file.Reports')}}</span></a>
                ...
                <ul id="report" class="collapse list-unstyled ">
                  ...
                  @if($sales_report_active)
                  <li id="sales-report-menu">
                    {!! Form::open(['route' => 'report.sales', 'method' => 'post', 'id' => 'sales-report-form']) !!}
                    <input type="hidden" name="start_date" value="1988-04-18" />
                    <input type="hidden" name="end_date" value="{{date('Y-m-d')}}" />
                    <input type="hidden" name="biller_id" value="0" />
                    <a id="sales-link" href="">{{trans('file.Sales Report')}}</a>
                    {!! Form::close() !!}
                  </li>
                  @endif
                  @if($purchases_report_active)
                  <li id="purchases-report-menu">
                    {!! Form::open(['route' => 'report.purchases', 'method' => 'post', 'id' => 'purchases-report-form']) !!}
                    <input type="hidden" name="start_date" value="1988-04-18" />
                    <input type="hidden" name="end_date" value="{{date('Y-m-d')}}" />
                    <input type="hidden" name="supplier_id" value="0" />
                    <a id="purchases-link" href="">{{trans('file.Purchases Report')}}</a>
                    {!! Form::close() !!}
                  </li>
                  @endif
          ...
          <div class="container-fluid">
            <div class="navbar-holder d-flex align-items-center justify-content-between">
              ...
              <ul class="nav-menu list-unstyled d-flex flex-md-row align-items-md-center">
                ...
                ?>
                @if($add_permission_active)
                <li class="nav-item"><a class="dropdown-item" href="{{route('sale.pos')}}"><i class="fa fa-th-large"></i><span> Factura</span></a></li>
                @endif            
                @if($alert_product > 0)
                <li class="nav-item">
    ...
    <div class="page">
      ...
      <!-- truck modal -->
      <div id="truck-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                  <p class="italic"><small>{{trans('file.The field labels marked with * are required input fields')}}.</small></p>
                    {!! Form::open(['route' => 'trucks.store', 'method' => 'post']) !!}
                      <?php 
                      ...
                      <div class="row">
                        ...
                        <div class="col-md-6 form-group">
                            <label><strong>{{trans('file.Maximum Capacity')}} *</strong></label>
                            <input type="number" name="capacity" step="any" required class="form-control">
                        </div>
                        <div class="col-md-6 form-group">
                            <label><strong>{{trans('file.Unit')}} *</strong></label>
                            <select name="unit_id" class="selectpicker form-control" required data-live-search="true" data-live-search-style="begins" title="Select unit...">
      ...
      <div id="transfer-money-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
        <div role="document" class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 id="exampleModalLabel" class="modal-title">{{trans('file.Transfer Money')}}</h5>
                    <button type="button" data-dismiss="modal" aria-label="Close" class="close"><span aria-hidden="true">×</span></button>
                </div>
                <div class="modal-body">
                  <p class="italic"><small>{{trans('file.The field labels marked with * are required input fields')}}.</small></p>
                    {!! Form::open(['route' => 'accounts.transferMoney', 'method' => 'post']) !!}
                      <div class="row">
                          <div class="col-md-6 form-group">
                              <label><strong> {{trans('file.From Account')}} *</strong></label>
                          ...
                          <div class="col-md-6 form-group">
                              <label><strong> {{trans('file.To Account')}} *</strong></label>
    ...
    <script type="text/javascript">
      ...
      $("a#transfer-money").click(function(e){
        e.preventDefault();
        $('#transfer-money-modal').modal();
      });
